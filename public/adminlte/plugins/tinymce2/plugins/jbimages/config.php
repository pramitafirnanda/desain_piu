...
| -------------------------------------------------------------------*/
	...
	$folderYear = $_SERVER['DOCUMENT_ROOT']."/uploads/post/" . $year . "/";
	$folderMonth = $_SERVER['DOCUMENT_ROOT']."/uploads/post/" . $year . "/".$month."/";
	$folderDay= $_SERVER['DOCUMENT_ROOT']."/uploads/post/" . $year . "/".$month."/".$day."/";
	...
	if (!file_exists($folderDay)) {
	    mkdir($folderDay, 0777, true);
	}
	$filename = "/uploads/post/" . $year . "/".$month."/".$day;

	$config['img_path'] = $filename; // Relative to domain name
